...
class GeneticAlgorithmGPAFPO:
    ...
    def __init__(self, args, mut_rate, inbred_threshold=None):
        ...
        self.random_injection = args.random_injection
    ...
    def run(self, fitness_function):
        ...
        for gen in range(self.generations):
            ...
            # AFPO-style random injection of individuals
            random_individuals = []
            n_injections = int(self.pop_size * self.random_injection)
            for _ in range(n_injections):
                # Introduce new random individuals to maintain population size if inbreeding is not allowed
                new_individual = Individual(self.args, fitness_function=self.fitness_function, init_method="full")
                random_individuals.append(new_individual)
                
                
            # print(f"HBC matings: {count_pos}. Random adds: {count_neg}.")
            self.random_list.append(random_count)
            ...
            combined_population = next_population[:lambda_pop] + self.population + random_individuals
            combined_population.sort(key=lambda ind: ind.fitness, reverse=True)
            
            # print(f"The combined population has: {len(combined_population)} individuals.")

            # Update the population
            self.population = combined_population[:self.pop_size]
            # print(f"The final population has: {len(self.population)} individuals.")
            ...
            if (gen + 1) % 10 == 0:
                ...
                print(f"On Average. {np.mean(self.random_list)} are randomly introduced.\n")            
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.inbred_threshold == 1:
            ...
            term3 = f"RandomInjection_{args.random_injection}_PopSize:{args.pop_size}_InThres:{args.inbred_threshold}_Mrates:{args.mutation_rate}_Gens:{args.generations}_TourSize:{args.tournament_size}_MaxD:{args.max_depth}_InitD:{args.initial_depth}" 
        ...
        if args.inbred_threshold == 1:
            ...
            results_inbreeding = exp.test_multiple_runs_function_gp_based_afpo(args, gp_landscape, None)
            util.save_accuracy(results_inbreeding, f"{args.config_plot}_inbreeding.npy")
        else:
            print("Running GA with NO Inbreeding Mating...")
            results_no_inbreeding = exp.test_multiple_runs_function_gp_based_afpo(args, gp_landscape, args.inbred_threshold)
