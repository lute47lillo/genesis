...
class GeneticAlgorithmGPTesting:
    ...
    def can_mate(self, ind1, ind2, inbred_threshold):
        """
            Definition
            -----------
                InbreedUnblock -> Inbreeding support mechanism based of average pairwise distance between trees.
                
                Example:  # Tree 1: (x + 1) -> tree1 = Node('-', [Node('x'), Node('1')])
                          # Tree 2: (x + 2) -> tree2 = Node('+', [Node('x'), Node('2')])
                          have distance of 2. 
                          If inbred_threshold = None, 1 or 2 they could NOT generate offspring.
                          If inbred_threshold = 3 or larger they could generate offspring.
        """
        distance = self.compute_trees_distance(ind1.tree, ind2.tree)
        return distance < inbred_threshold # Essentially InbreedUnblock lets 2 individuals mate if the distance is lower than the threshold, so, if they are similar.
